...
public class SchemaGeneratorTest {
    ...
    String schema(Class<?> cls) {
        ...
        if (String.class.equals(cls)) {
            ...
            result =
                String.format("SchemaBuilder.array(%s).optional().doc(\"\").build()", childSchema);
        } else if (Map.class.isAssignableFrom(cls)) {
            result = "SchemaBuilder.map(Schema.STRING_SCHEMA, SCHEMA_MEDIA_ENTITY_SIZE)";

        } else {
            result = "SCHEMA_" +
                CaseFormat.UPPER_CAMEL.to(CaseFormat.UPPER_UNDERSCORE, cls.getSimpleName())
                    .replace('$', '_');
        }
    ...
    void processClass(Class<?> cls, StringBuilder builder) {
        ...
        if (null == cls.getDeclaringClass()) {
            schemaConstantName =
                "SCHEMA_" +
                    CaseFormat.UPPER_CAMEL.to(CaseFormat.UPPER_UNDERSCORE, cls.getSimpleName());
            schemaName =
                String.format("com.thinksky.kafka.twitter.stream.connect.%s", cls.getSimpleName());
            typeName = cls.getSimpleName();
            convertMethodName = String.format("convert%s", cls.getSimpleName());
        } else {
            schemaConstantName = "SCHEMA_" + CaseFormat.UPPER_CAMEL.to(CaseFormat.UPPER_UNDERSCORE,
                cls.getDeclaringClass().getSimpleName() + cls.getSimpleName());
            typeName =
                String
                    .format("%s.%s", cls.getDeclaringClass().getSimpleName(), cls.getSimpleName());
            schemaName = String
                .format("com.thinksky.kafka.twitter.stream.connect.%s.%s", cls.getSimpleName(),
                    cls.getDeclaringClass().getSimpleName());
            convertMethodName = String
                .format("convert%s%s", cls.getDeclaringClass().getSimpleName(),
                    cls.getSimpleName());
        }
        ...
        methods.clear();
        String variableName =
            CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_CAMEL, cls.getSimpleName());
        builder.append(
            String
                .format("static Struct %s(%s %s) {\n", convertMethodName, typeName, variableName));
        ...
        for (Method method : cls.getMethods()) {
            ...
            builder.append(
                String.format("\n    .put(\"%s\", %s.%s())", methodName, variableName,
                    method.getName()));
        }
        builder.append(";\n  }\n");
        ...
        builder.append("\n");
        builder
            .append(String.format("public static List<Struct> convert(%s[] items) {\n", typeName));

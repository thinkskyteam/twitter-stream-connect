...
public class TwitterSourceTask extends SourceTask implements StatusListener {
    ...
    public void start(Map<String, String> map) {
        ...
        TwitterStreamFactory twitterStreamFactory =
            new TwitterStreamFactory(this.config.configuration());
        ...
        if (!this.config.filterUserIds.isEmpty()) {
            ...
            log.info("Setting up filters. userIds = {}",
                Joiner.on(", ").join(this.config.filterUserIds));
    ...
    public void onDeletionNotice(StatusDeletionNotice statusDeletionNotice) {
        ...
        try {
            ...
            SourceRecord record = new SourceRecord(sourcePartition, sourceOffset, this.config.topic,
                StatusConverter.SCHEMA_STATUS_DELETION_NOTICE_KEY, keyStruct, null, null);
            this.messageQueue.add(record);
        } catch (Exception ex) {
            if (log.isErrorEnabled()) {
                log.error("Exception thrown", ex);
    ...
    public void onStallWarning(StallWarning stallWarning) {
        if (log.isWarnEnabled()) {
            log.warn("code = '{}' percentFull = '{}' - {}", stallWarning.getCode(),
                stallWarning.getPercentFull(), stallWarning.getMessage());
        }
    }
